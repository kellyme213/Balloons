...
Vec3f Cloth::calculateForce(int i1, int j1, int i2, int j2, double k_constant)
{
    if (i1 < 0 || i1 >= nx || i2 < 0 || i2 >= nx || j1 < 0 || j1 >= ny || j2 < 0 || j2 >= ny)
    {
        return Vec3f(0, 0, 0);
    }
    ClothParticle p1 = getParticle(i1, j1);
    ClothParticle p2 = getParticle(i2, j2);
...
bool Cloth::particleExists(int i, int j)
{
    return i >= 0 && i < nx && j >= 0 && j < ny;
}

double Cloth::isStretched(ClothParticle& p1, ClothParticle& p2, double k_constant)
{
    Vec3f originalLength = p2.original_position - p1.original_position;
    Vec3f currentLength = p2.position - p1.position;
...
static int num = 0;
void Cloth::provotCorrection(int i1, int j1, int i2, int j2, double k_constant)
{
